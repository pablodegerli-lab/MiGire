...
Version 11.1
 ...
 La conexion tiene las siguiente propiedades:
  ...
  ReconectarSiError   : Activa/Desactiva la re-conexion del datamodulo.

  FormatoAntiguoCif   : Activa/Desactiva la conexion automatica con el viejo archivo de claves del datamodulo. Tiene que estar activada
                        la conexion directa con  propiedad ConexionDirecta.
...
uses
  ...
  stdctrls, registry, Db, ADODB, DBTables, inifiles, define, setearol,
  AppEvnts, ComObj;

resourcestring
  SEVersion = 'Ver. 11.2.10';
  SEstadoPrim = 'OPEN';
  SEstadoExpi = 'EXPIRED';
...
type
  ...
  TADOConexion = class(TDataModule)
    ...
    procedure aeGireException(Sender: TObject; E: Exception);
  private
    ClaveFin: boolean;
    MsgError: string;
    CambioClave: string;
    ...
    FReconectarSiError: Boolean;
    FFormatoAntiguoCif: Boolean;
    ...
    procedure SetReconectarSiError(const Value: Boolean);
  protected
  ...
  public
    ...
    property ReconectarSiError: Boolean read FReconectarSiError write SetReconectarSiError  stored False;
    property FormatoAntiguoCif: Boolean read FFormatoAntiguoCif write FFormatoAntiguoCif  stored False;
  end;

Const
  E_ADO_FALLO = -2147467259;
...
for Recorre:= 0 to ParamCount  do
 begin
  ...
  if Pos('-c', ParamStr(Recorre))<> 0 then
     FormatoAntiguoCif := True;
 end;
  ...
  if ConexionDirecta then
   begin
     if FormatoAntiguoCif then
      ...
      try
//        setearol.TSeteaRoles DatosConexion(ArchivoClave, FNombreUsuario, Ambito, FClaveUsuario);
        GrabarLog('Se cargaron exitosamente los datos de conexion del archivo cif...');
      except
        on e: exception do
         begin
          GrabarLog('Error!!...'+e.Message);
          raise Exception.Create('Problemas para extraer los datos desde el archivo. Error: '+e.Message);
         end;
      end;
   end;
...
procedure TADOConexion.aeGireException(Sender: TObject;
  E: Exception);
var
  EO: EOleException;
begin
  if E is EOLEException then
   begin
    EO := EOleException(E);
    //Error de conexion... fallo de red?
    if (FReconectarSiError) and (EO.ErrorCode = E_ADO_FALLO) then
     begin
      try
        try
          ADOConec.Close;
        ...
        AbrirConexion(ADOConec);
        //ADOConec.Open;
        if not FConexionDirecta then
          application.MessageBox(Pchar('Conexión re-establecida luego de micro corte, por favor, continue!'),PChar('Atención!!!'),MB_ICONINFORMATION)
        else
          GrabarLog('Conexión re-establecida luego de micro corte...');
      except
        on E:Exception do
          if not FConexionDirecta then
            application.MessageBox(Pchar(Format('La conexión al servidor a fallado.'+ #13#10
            + 'Por favor, reintente más tarde'#13'Error reportado: %s', [E.Message])),Pchar('Error de Red'),MB_ICONERROR)
          else
           GrabarLog('La conexión al servidor a fallado. Error reportado: ' + E.Message);
      end;
     end
    else
    if not FConexionDirecta then
      application.MessageBox(Pchar(E.Message + ' ' + IntToStr(EO.ErrorCode)),Pchar('Error de Conexión'),MB_ICONERROR)
    else
      GrabarLog('Error de Conexión reportado: ' + E.Message);
   end
  else
    if not FConexionDirecta then
      application.MessageBox(Pchar(E.ClassName + #13 + E.Message),Pchar('Error de Aplicación'),MB_ICONERROR)
    else
      GrabarLog('Error de Aplicación reportado: ' + E.Message);
end;
...
procedure TADOConexion.SetReconectarSiError(const Value: Boolean);
begin
  FReconectarSiError := Value;
end;
