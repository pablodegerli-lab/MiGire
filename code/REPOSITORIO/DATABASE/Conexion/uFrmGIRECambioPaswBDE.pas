...
type
  ...
  TFrmGIRECambioPaswBDE = class(TFrmGIREDlg)
...
procedure TFrmGIRECambioPaswBDE.FormActivate(Sender: TObject);
begin
  inherited;
  try
...
procedure TFrmGIRECambioPaswBDE.edPaswAnteriorExit(Sender: TObject);
begin
  inherited;
...
function TFrmGIRECambioPaswBDE.VerificaClaves(Sender: TObject) : boolean;
begin
 result:=true;
...
procedure TFrmGIRECambioPaswBDE.AceptarExecute(Sender: TObject);
...
procedure TFrmGIRECambioPaswBDE.edPaswAnteriorEnter(Sender: TObject);
begin
  inherited;
...
procedure TFrmGIRECambioPaswBDE.SetGireEnlace(const Value: TGIREConexion);
begin
  FGireEnlace := Value;
end;

function TFrmGIRECambioPaswBDE.URLFormat(pURL: String): String;
...
function TFrmGIRECambioPaswBDE.GetURL: String;
begin
    try
        try
            ...
            qryCambioPasw.sql.Add('select valor from gire.parametros');
            qryCambioPasw.sql.Add('where nombre = ''URL_ADMIN_CLAVES'' ');
            qryCambioPasw.Open;
            if ( qryCambioPasw.RecordCount = 0 ) or ( Empty(qryCambioPasw.FieldValues['valor']) ) then
                raise Exception.Create('Error en la Captura del parametro URL');
            result := qryCambioPasw.FieldValues['valor'];
        finally
            qryCambioPasw.Close
        end;
    except
